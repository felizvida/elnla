...
- No-extension attachment limitation observed on May 14, 2026: LabArchives accepted no-extension uploads named `RAW_EXPORT` and `README_NO_EXTENSION`, but `notebooks::notebook_backup` omitted their `original/` payloads, causing ELNLA verification to fail with 267/271 originals. The seed now uses suffixed versions so the clean testing notebook can pass full-size backup verification.
