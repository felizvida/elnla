...
- Expanded integration stress seed on May 14, 2026: `scripts/labarchives_seed_bio_test_notebook.py` now reuses the local integration notebook by default and adds a dated "Molecular Biology Web Lab Stress Test" subtree with 24 verbose pages, 129 text entries, 24 comments, and 51 attachments. The fixtures cover qPCR, RNA-seq counts, flow cytometry, microscopy, cloning, Sanger/amplicon review, freezer inventory, CAPA, external storage links, scripts, notebooks, PDF reports, PNG/SVG images, FASTQ/FASTA/VCF/GFF3/GenBank, and JSON/XML/CSV/TSV exports. Generated payload files and returned IDs stay under ignored `local_credentials/`.
