...
- Request backups with `json=true` and omit `no_attachments=true`. Passing `no_attachments=true` produces a lighter archive but breaks BenchVault's evidence and Save Original goals because full-size original files are absent.
...
- Save Original means copying the backed-up original file to a user-selected folder, not opening or modifying the backup in place. Sanitize path separators and control characters from attachment names, then add suffixes such as `name (1).ext` to avoid overwriting existing files.
- After saving, compare the copied file size to the backup metadata and delete the copied file if the size does not match. This catches interrupted copies and path-resolution mistakes.
...
- Viewer Save Original rule: render sidecars include each attachment original's backup-relative path when the original file is present. The read-only viewer copies attachments to a user-selected folder without overwriting existing files; legacy render files fall back to `extracted/notebook/attachments/<entry-part-id>/.../original/<filename>`.
- Viewer attachment format rule: LabArchives allows arbitrary notebook attachments, with direct/cloud viewers for text, PDF, Office, common browser images, Jupyter `.ipynb`, and SnapGene/sequence families. BenchVault should preserve every original file, inline-preview safe local formats, treat HTML/SVG as source text, and classify Office/PDF/SnapGene/TIFF/chemical/media/archive/custom files with Save Original affordances until dedicated renderers are added.
